...
`GET /api/venues?city=Алматы` — все площадки города, по возрастанию `price_from_kzt`, затем `id`. Без `city` возвращает площадки всех городов. Ответ: `{venues: [...], availability_checked: false}`. В отличие от подбора, обзор не ограничен тремя и не фильтруется по дате, бюджету, формату, языку или длительности; интерфейс явно обозначает это и не обещает доступность. Неизвестный город возвращает пустой список.

Элемент: `id`, `name` (дословное название перед тире в описании, иначе имя профиля), `profile_name` (исходное анонимное имя), `category`, `categories`, `city`, `price_from_kzt`, `description`, `price_imputed`, `city_imputed`, `synthetic`. Координат в ответе нет. Демонстрационная география отдельно описана в [VENUE_MAP.md](VENUE_MAP.md).

У карточек `/api/recommend` добавлено `venue_name`: название для площадок, `null` для остальных категорий. Поле `name` и все ограничения подбора сохранены. Для заголовка можно использовать `venue_name || name`.
